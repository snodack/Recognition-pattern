...
def train(model_name, epoch = 100, batch = 8, dataset = "my_dataset",):
    global model
    input, targets= tfds.load('my_dataset', split = 'train',batch_size=-1, shuffle_files = True, as_supervised = True)
    input/=255
    model = Sequential([
    ...
    while i < len(symbols):
        ...
        if current_symbol == "sqrt":
            ...
            if i != 0:
                ...
                elif symbols[i-1].isdigit() and char_handler.check_power(contours[i-1],contours[i]):
...
def predict_image(path):
    ...
    for i in range(len(contours)):
        c = model.predict(split.blank_image(contours[i]).reshape(1, out_size*out_size))
...
#train("new_big_model.h5", epoch= 20)